...
1. From the MSYS2 shell, install these packages:
   ```
   pacman -S \
       mingw-w64-x86_64-{gcc,cmake,make,python2,pkg-config,ninja,diffutils}
   ```
...
```
sudo yum -y install ninja-build cmake gcc gcc-c++ make pkgconfig unzip patch gettext curl
```
...
```sh
doas pkg_add gmake cmake unzip curl
```
