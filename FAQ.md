...
- [General questions](#general-questions)
    - [Where should I put my config (`vimrc`)?](#where-should-i-put-my-config-vimrc)
    - [How stable is the development (pre-release) version?](#how-stable-is-the-development-pre-release-version)
    - [Can I use Ruby-based Vim plugins (e.g. LustyExplorer)?](#can-i-use-ruby-based-vim-plugins-eg-lustyexplorer)
    - [Can I use Lua-based Vim plugins (e.g. neocomplete)?](#can-i-use-lua-based-vim-plugins-eg-neocomplete)
    - [How can I use "true color" in the terminal?](#how-can-i-use-true-color-in-the-terminal)
    - [Nvim shows weird symbols (`�[2 q`) when changing modes](#nvim-shows-weird-symbols-%EF%BF%BD2-q-when-changing-modes)
    - [How to change cursor _shape_ in the terminal?](#how-to-change-cursor-_shape_-in-the-terminal)
    - [Flick _cursor_ when use neovim under tmux?](#flick-_cursor_-when-use-neovim-under-tmux)
    - [How to change cursor _color_ in the terminal?](#how-to-change-cursor-_color_-in-the-terminal)
    - [Cursor style isn't restored after exiting or suspending and resuming Nvim](#cursor-style-isnt-restored-after-exiting-or-suspending-and-resuming-nvim)
    - [Cursor shape doesn't change in tmux](#cursor-shape-doesnt-change-in-tmux)
    - [Is Windows supported?](#is-windows-supported)
    - [How to use the Windows clipboard from WSL?](#how-to-use-the-windows-clipboard-from-wsl)
    - [What happened to --remote and friends?](#what-happened-to---remote-and-friends)
- [Runtime issues](#runtime-issues)
    - [Copying to X11 primary selection with the mouse doesn't work](#copying-to-x11-primary-selection-with-the-mouse-doesnt-work)
    - [My CTRL-H mapping doesn't work](#my-ctrl-h-mapping-doesnt-work)
    - [`<Home>` or some other "special" key doesn't work](#home-or-some-other-special-key-doesnt-work)
    - [`:!` and `system()` do weird things with interactive processes](#-and-system-do-weird-things-with-interactive-processes)
    - [Python support isn't working](#python-support-isnt-working)
    - [`:checkhealth` reports `E5009: Invalid $VIMRUNTIME`](#checkhealth-reports-e5009-invalid-vimruntime)
    - [Neovim can't find its runtime](#neovim-cant-find-its-runtime)
    - [`E518: Unknown option: [option]`](#e518-unknown-option-option)
    - [Neovim is slow](#neovim-is-slow)
    - [Colors aren't displayed correctly](#colors-arent-displayed-correctly)
    - [Neovim can't read UTF-8 characters](#neovim-cant-read-utf-8-characters)
    ...
    - [Calling `inputlist()`, `echomsg`, etc... in filetype plugins and `autocmd` does not work](#calling-inputlist-echomsg-etc-in-filetype-plugins-and-autocmd-does-not-work)
    - [`g:clipboard` settings are not used.](#gclipboard-settings-are-not-used)
- [Installation issues](#installation-issues)
    - [Generating helptags failed](#generating-helptags-failed)
- [Build issues](#build-issues)
    - [General build issues](#general-build-issues)
    - [Proxy issues #2482](#proxy-issues-2482)
    - [Settings in `local.mk` don't take effect](#settings-in-localmk-dont-take-effect)
    - [CMake errors](#cmake-errors)
    - [Lua packages](#lua-packages)
    - [Anaconda error](#anaconda-error)
- [Develop](#develop)
    - [Plugins](#plugins)
    - [Tools](#tools)
- [Debug](#debug)
    - [Backtrace (Linux)](#backtrace-linux)
    - [Backtrace (macOS / OSX)](#backtrace-macos--osx)
    - [Using `gdb` to step through functional tests](#using-gdb-to-step-through-functional-tests)
    - [Using `lldb` to step through unit tests](#using-lldb-to-step-through-unit-tests)
    - [Using `gdb`](#using-gdb)
    - [Using `gdbserver`](#using-gdbserver)
    - [Using `gdbserver` in `tmux`](#using-gdbserver-in-tmux)
    - [Log file location](#log-file-location)
- [Design](#design)
    - [Why not use JSON for RPC?](#why-not-use-json-for-rpc)
    - [Why embed Lua instead of X?](#why-embed-lua-instead-of-x)
    - [Why Lua 5.1 instead of Lua 5.3+?](#why-lua-51-instead-of-lua-53)
    - [Will Neovim translate VimL to Lua, instead of executing VimL directly?](#will-neovim-translate-viml-to-lua-instead-of-executing-viml-directly)
